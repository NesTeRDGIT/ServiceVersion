...
{
    public class MYBDOracle : IRepository
    {
        ...
        public MYBDOracle(string _con, TableInfo _H_ZGLV, TableInfo _H_SCHET, TableInfo _H_SANK, TableInfo _H_CODE_EXP, TableInfo _H_PAC, TableInfo _H_ZAP,
...
:K_FR, :WEI, :HEI, :BSA,:EXTR, :SUM_MP)", con);
                ...
                cmd.Parameters.Add("WEI", OracleDbType.Decimal, Items.Select(x => x.ONK_SL != null ? x.ONK_SL.WEI ?? (object)DBNull.Value : (x.WEI?? (object)DBNull.Value)).ToArray(), ParameterDirection.Input);
...
(:SANK_ID,:SLUCH_ID,:SLUCH_Z_ID,:S_CODE,:S_COM,:S_IST, :S_OSN, :S_SUM, :S_TIP, :S_YEAR,:S_MONTH,:S_TEM, :S_PLAN, :S_FINE,:S_IDSERV,:S_ZGLV_ID,:DATE_ACT,:NUM_ACT,:SL_ID)", con);
        ...
        void InsertLEK_PR_H(List<LEK_PR_H> Items)
        {
            ...
            {
                if (!Items.Any()) return;
                cmd = NewOracleCommand($@"insert into {LEK_PR_H_TBL.FullTableName}  (SLUCH_ID,CODE_SH,COD_MARK,DATA_INJ,REGNUM,COL_INJ,DOSE_INJ,ED_IZM,METHOD_INJ) values (:SLUCH_ID,:CODE_SH,:COD_MARK,:DATA_INJ,:REGNUM,:COL_INJ,:DOSE_INJ,:ED_IZM,:METHOD_INJ)", con);
                cmd.ArrayBindCount = Items.Count;
                cmd.BindByName = true;
                ...
                cmd.Parameters.Add("DOSE_INJ", OracleDbType.Decimal, Items.Select(x => x.LEK_DOSE != null ? x.LEK_DOSE.DOSE_INJ : (object)DBNull.Value).ToArray(), ParameterDirection.Input);
                cmd.Parameters.Add("ED_IZM", OracleDbType.Varchar2, Items.Select(x => x.LEK_DOSE != null ? x.LEK_DOSE.ED_IZM : (object)DBNull.Value).ToArray(), ParameterDirection.Input);
                cmd.Parameters.Add("METHOD_INJ", OracleDbType.Varchar2, Items.Select(x => x.LEK_DOSE != null ? x.LEK_DOSE.METHOD_INJ : (object)DBNull.Value).ToArray(), ParameterDirection.Input);
...
where zs.idcase  in ({string.Join(",", idcase)}) and s.zglv_id = :zglv_id", con);
        ...
        */
                        ...
                        for (var i = 0; i < l.Count; i++)
                        {
                            ...
                            if (proc.STATE)
                            {
                                ...
                                foreach (var par in proc.listParam)
                                {
                                    ...
                                    cmd.Parameters.Add(par.Name, par.Type.ToOracleDbType(), value, ParameterDirection.Input);
                                }
        ...
        private object GetValueOrclParam(OrclParam par)
        {
            ...
            if (par.Type.ToOracleDbType() == OracleDbType.Int32)
