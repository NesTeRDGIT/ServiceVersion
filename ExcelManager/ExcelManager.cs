...
namespace ExcelManager
{
    ...
    public class ExcelOpenXML : IDisposable
    {
        ...
        public void Save()
        {
            ...
            var ex = workbookPart.Workbook.GetFirstChild<DocumentFormat.OpenXml.Spreadsheet.WorkbookExtensionList>();
            var ex1 = workbookPart.WorkbookStylesPart.Stylesheet.GetFirstChild<DocumentFormat.OpenXml.Spreadsheet.StylesheetExtensionList>();
            ex?.Remove();
            ex1?.Remove();
            document.Save();
            this.Dispose();  
        ...
        private static double GetWidth(System.Drawing.Font stringFont, string text)
        {
            ...
            var textSize = System.Windows.Forms.TextRenderer.MeasureText(text, stringFont);
            var width = (double)(((textSize.Width / (double)7) * 256) - (128 / 7)) / 256;
